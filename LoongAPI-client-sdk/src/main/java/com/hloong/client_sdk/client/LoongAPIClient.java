package com.hloong.client_sdk.client;
...
import com.hloong.client_sdk.exception.ErrorResponse;
import com.hloong.client_sdk.model.BaseRequest;
import com.hloong.client_sdk.model.entity.User;
import com.hloong.client_sdk.model.enums.UrlToMethodEnum;
import com.hloong.client_sdk.model.params.HoroscopeParams;
import com.hloong.client_sdk.model.params.PublicIpParams;
import com.hloong.client_sdk.model.response.*;
import com.hloong.client_sdk.utils.SignUtils;
...
public class LoongAPIClient {
    ...
    public LoongAPIClient(String accessKey, String secretKey) {
     ...
     * @return 用户名
     */
    public UserResponse getUsernameByPost(User user) throws BusinessException {
     ...
     * @return 微博实时热搜榜
     */
    public WeiboHotSearchResponse getWeiboHotSearch() throws BusinessException {
     ...
     * @return 星座运势
     */
    public HoroscopeResponse getHoroscope(HoroscopeParams horoscope) throws BusinessException {
     ...
     * @return 公网ip
     */
    public PublicIpResponse getPublicIp(PublicIpParams publicIp) throws BusinessException {
     ...
     */
    public RandomWallpaperResponse getRandomWallpaper() throws BusinessException {
     ...
     * @throws BusinessException
     */
    public LoveTalkResponse getLoveTalk() throws BusinessException {
     ...
     * @throws BusinessException
     */
    public WeatherResponse getWeather() throws BusinessException {
     ...
     * @throws BusinessException
     */
    public Object parseAddressAndCallInterface(BaseRequest baseRequest) throws BusinessException {
        ...
        Class<?> clazz = LoongAPIClient.class;
        Object result = null;
        try {
            ...
            if (path.equals(UrlToMethodEnum.publicIp.getPath())) {
                ...
                if (ipAddress == null || ipAddress.isEmpty()) {
                    throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "获取公网ip失败");
                }
        ...
        if (ObjUtil.isEmpty(result)) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "未找到Url对应资源");
        }
     ...
     * @throws BusinessException
     */
    private Object invokeMethod(String methodName) throws BusinessException {
     ...
     * @throws BusinessException
     */
    private Object invokeMethod(String methodName, Map<String, Object> params, Class<?> paramsType) throws BusinessException {
        try {
            Class<?> clazz = LoongAPIClient.class;
            if (params == null) {
            ...
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "通过url未找到对应方法");
        } catch (Exception e) {
            // 处理异常
     ...
     */
    public <T> T sendRequest(String url, String method, Object params, Class<T> responseType) throws BusinessException {
        HttpRequest request;
        ...
        switch (method) {
            ...
            default:
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "请求方式有误");
        }
        ...
        log.info("Loong-API开放平台网关返回responseBody:{}", responseBody);
        if (response.getStatus() != 200) {
            // 处理网关返回异常
            ErrorResponse errorResponse = JSONUtil.toBean(responseBody, ErrorResponse.class);
            throw new BusinessException(errorResponse.getCode(), errorResponse.getMessage());
        }
        // 获取响应体
     ...
     * 检查配置
     *
     * @param loongAPIClient api客户端
     * @throws BusinessException 业务异常
     */
    public void checkConfig(LoongAPIClient loongAPIClient) throws BusinessException {
        if (StrUtil.isBlank(loongAPIClient.getSecretKey()) && StrUtil.isBlank(loongAPIClient.getSecretKey())) {
    ...
    private Map<String, String> getHeaderMap(String body) {
        ...
        headerMap.put("X-SecretId", secretKey);
